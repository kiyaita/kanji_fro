...
#def connect_hands(tracks, hand1, hand2):
...
connected = []#つながった２つの手のidを[[id1,id2],[id3,id4]]の形で保存しておきたい
connect_distance=200     ###############################a"""後で計算方式にするbboxのサイズに応じて近づいた判定の距離変えたいかも（カメラ固定するから一定値でも問題ない？）
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            ...
            for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                ...
                """
                for j, (idA, idB) in enumerate(connected):
                ...
                for i,track1 in enumerate(tracks):#一定距離近づいたら繋げる、「一定距離離れたらidのペアを外す」がないから動作不良
                    if any(track1[0] in connected_pair for connected_pair in connected):
                        continue
                    track1_centor = ((track1.box[2]+track1.box[0])/2 ,(track1.box[3]+track1.box[1])/2 )
                    closest_dist = connect_distance
                    closest_hand = None
                    for j,track2 in enumerate(tracks):
                        if i == j:
                            continue
                        if any(track2[0] in connected_pair for connected_pair in connected):
                            continue
                        track2_centor = ((track2.box[2]+track2.box[0])/2 ,(track2.box[3]+track2.box[1])/2 )
                        dist = np.sqrt((track1_centor[0] - track2_centor[0])**2 + (track1_centor[1] - track2_centor[1])**2)
                        ...
                        if closest_hand is not None:
                            connected.append([track1[0],closest_hand[0]])
                    
                for j, (idA, idB) in enumerate(connected):
                    trackA = [track for track in tracks if track.id == idA]
                    trackB = [track for track in tracks if track.id == idB]
                    
                    if not trackA: print("notAtrue")
                    if not trackB: print("notBtrue")
                    if trackA and trackB:
                        
                        trackA ,trackB =trackA[0], trackB[0]
                        
                        trackA_centor = ((trackA.box[2]+trackA.box[0])/2 ,(trackA.box[3]+trackA.box[1])/2 )
                        trackB_centor = ((trackB.box[2]+trackB.box[0])/2 ,(trackB.box[3]+trackB.box[1])/2 )

                        cv2.line(image, (int(trackA_centor[0]), int(trackA_centor[1])), (int(trackB_centor[0]), int(trackB_centor[1])), (0, 255, 0), 100)
                        """cv2.circle(image, (int(trackA_centor[0]), int(trackA_centor[1])), 100, (255, 255, 255), -1)
                        cv2.circle(image, (int(trackB_centor[0]), int(trackB_centor[1])), 200, (255, 255, 255), -1)
                        """
                        print("CC")
                ...
                for track in tracks:
                    ...
                    if track == tracks[0]:#謎の画像全部同じになる対策、多分ポインタ関係で変なんやと思う
